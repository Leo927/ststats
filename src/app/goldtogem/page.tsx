...
export default function Home() {
  const [goldtogem, setgoldtogem] = useState<any[]>([]);
  ...
  useEffect(() => {
    fetch("/api/market/goldtogem")
      .then((res) => res.json())
      .then(getQualities)
      .then(groupUidWithTag1)
      .then((data) => setgoldtogem(data)).finally(() => console.log("Updated"));
  }, []);
  ...
  return (
    <Box className="items-center">
      <AppRouterCacheProvider>
        <Box>
          <DataGrid rows={goldtogem} columns={columns} getRowId={i => i.uid}>
          </DataGrid>
